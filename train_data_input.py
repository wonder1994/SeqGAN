...
def main(unused_argv):
    ...
    test_likelihood_data_loader = Gen_Data_loader(config_gen.gen_batch_size, config_gen.sequence_length)
    ...
    #generator = generator(config=config_gen, params=target_params) # The oracle model
    ...
    #generate_samples(sess, generator, config_train.batch_size, config_train.generated_num, config_train.train_file)
    ...
    for epoch in range(config_train.pretrained_epoch_num):
        ...
        if epoch % config_train.test_per_epoch == 0:
            ...
            test_likelihood_data_loader.create_batches(config_train.test_file)
            test_loss = generator_loss(sess, generator, test_likelihood_data_loader)
        ...
        if config_train.reinforce:
            ...
            for total_batch in range(config_train.total_batch):
                ...
                if total_batch % config_train.test_per_epoch == 0 or total_batch == config_train.total_batch - 1:
                    ...
                    test_likelihood_data_loader.create_batches(config_train.test_file)
                    test_loss = generator_loss(sess, generator, test_likelihood_data_loader)
                    train_loss = generator_loss(sess, generator, gen_data_loader)
                    # buffer = 'epoch:\t' + str(total_batch) + '\tnll:\t' + str(test_loss) + '\n'
                    # print ('total_batch: ', total_batch, 'test_loss: ', test_loss)
                    bleu_test_reward = bleu_reward_eval(sess, bleu_reward, likelihood_data_loader)
                    buffer = 'epoch:\t' + str(total_batch + epoch) + '\tnll:\t' + str(test_loss) + '\tbleu:\t' + str(bleu_test_reward) + '\n'
                    print ('total_batch: ', total_batch + epoch, 'train_loss: ', train_loss, 'test_loss: ', test_loss, 'bleu_test_reward: ', bleu_test_reward)
                    log.write(buffer)
    log.close()
