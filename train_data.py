# add training file input for any dataset, change the nll of target lstm to that of current rnn.
import random
import os
...
def main(unused_argv):
    ...
    #Write data to config_train.train_file
    # generate_samples(sess, target_lstm, config_train.batch_size, config_train.generated_num, config_train.train_file)
    gen_data_loader.create_batches(config_train.train_file)
    # bleu reward function
    reference_corpus = []
    with open(config_train.train_file)as fin:
        for line in fin:
            line = line.strip()
            line = line.split()
    ...
    for epoch in range(config_train.pretrained_epoch_num):
        ...
        if epoch % config_train.test_per_epoch == 0:
            ...
            test_loss = generator_loss(sess, generator, likelihood_data_loader)
            #print ('pre-train epoch ', epoch, 'test_loss ', test_loss)
            #buffer = 'epoch:\t'+ str(epoch) + '\tnll:\t' + str(test_loss) + '\n'
            bleu_test_loss = bleu_loss(sess, bleu_reward, likelihood_data_loader)
            buffer = 'epoch:\t' + str(epoch) + '\tnll:\t' + str(test_loss) + '\tbleu:\t' + str(bleu_test_loss) + '\n'
            print ('pre-train epoch ', epoch, 'test_loss: ', test_loss, 'bleu_test_loss: ', bleu_test_loss)
            log.write(buffer)
    ...
    for t in range(config_train.dis_update_time_pre):
        ...
        dis_data_loader.load_train_data(config_train.train_file, config_train.negative_file)
    ...
    # start training with n-gram loss
        ...
        if config_train.reinforce:
            ...
            rollout_gen = rollout(config=config_gen, full_sequence=False)
            for total_batch in range(config_train.total_batch):
                for iter_gen in range(config_train.gen_update_time):
                    samples = sess.run(generator.sample_word_list_reshape)
                    feed = {"pred_seq_rollout:0":samples}
            ...
            for total_batch in range(config_train.total_batch):
                ...
                if total_batch % config_train.test_per_epoch == 0 or total_batch == config_train.total_batch - 1:
                    ...
                    # print ('total_batch: ', total_batch, 'test_loss: ', test_loss)
                    bleu_test_loss = bleu_loss(sess, bleu_reward, likelihood_data_loader)
                    buffer = 'epoch:\t' + str(total_batch) + '\tnll:\t' + str(test_loss) + '\tbleu:\t' + str(bleu_test_loss) + '\n'
                    print ('total_batch: ', total_batch, 'test_loss: ', test_loss, 'bleu_test_loss: ', bleu_test_loss)
                    log.write(buffer)
    log.close()
